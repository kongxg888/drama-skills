...
- 新剧本：从本轮开始就按可生产对白、动作、声音和连续性要求成稿，并在交给下游前做联动就绪检查。
- 已有剧本：按存量回溯处理，保留作者语言，只做用户点名或生产确实需要的定点修订。
...
8. 新剧本交下游前，逐场确认对白、标点、表演方式、声音事实、动作容量、关键持物和连续性都能被分镜承载；已有剧本则列出缺口，不借检查之名整集重写。
9. 局部修订保留无关段落；明确问题直接修正，只把真实剧情分叉交给用户。

## 新稿、存量稿与下游回流

- 剧本是对白、标点、动作事实、人物意图和因果的来源。H3 或其他生成端不得在生产 job 里改词。
- 如果生成结果只是语气、音量、节奏、构图或限制条件没有执行，优先修当前提示词；不要为了迁就生成结果改剧本。
- 如果发现台词本身不自然、标点改变语气、人物动机不成立或缺少必须事实，才回到本技能修剧本，并通知分镜与视频提示词 owner 刷新受影响内容。
- 剧本一旦改变台词、标点、动作事实、人物意图或连续性，相关分镜、图片/视频提示词和已准备/已确认 job 都标记为
  `stale/已过期`；不能继续沿用旧提示词或旧确认。
- 已生成媒体保留为旧版本证据，不因改稿自动删除或覆盖。

完整的层级判定、版本失效和回流记录见 `$short-drama` 的“剧本到生产的联动回流”规则。
