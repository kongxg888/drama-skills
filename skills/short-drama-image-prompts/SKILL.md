...
人物用途若是正式身份母版，默认读取[默认人物身份四视图母版](references/character-identity-master-default.md)：
使用一张无缝横向画布、四个固定视角和其中的真人质感、排除项与身份字段。该默认只约束
`character-identity-master`，不强制造型变体、状态图或镜头尾帧使用四视图；项目已确认的视觉方向
和创作者明确选择始终优先。候选图未被创作者选定前，不得写成正式身份母版。

地点/空间用途默认读取[默认场景空间母版](references/scene-space-master-default.md)，长期复用关键道具
用途默认读取[默认道具二宫格母版](references/prop-two-panel-master-default.md)。两者都默认使用
RunningHub G2、4K、横向 16:9；只出现一两次的临时场景或道具不建立独立母版，按 `prompt_only`
写入对应镜头或状态配方。不得把场景模板套给道具，也不得把道具二宫格套给地点空间。
...
- 正式身份母版固定使用默认四视图版式；只有 Look、状态或镜头用途才按用途选择视图数量。
- 长期复用地点/空间固定使用 `scene-space-master`；长期复用关键道具固定使用 `prop-two-panel-master`。
