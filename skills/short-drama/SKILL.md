...
新集和已有集的联动边界、下游问题回流以及版本失效规则，见 [creator-first 工作流](references/creator-workflow.md)
中的“剧本到生产的联动回流”章节。
...
新剧本从剧本阶段建立完整生产链；已有剧本先做存量对齐，不默认重写作者原文。图片或视频生成后发现问题时，先按提示词、分镜、剧本三层判断，再回到对应 owner 修订；任何上游改变都使受影响的下游提示词和已准备/已确认 job 标记为
`stale/已过期`，重新预览确认后才能继续生产。
...
7. 一旦发现下游生成结果与来源不一致，先按[剧本到生产的联动回流](references/script-to-production-feedback-loop.md)
   分类影响层，不在末端提示词里掩盖剧本或分镜问题；需要修改时回到对应 owner，并刷新所有受影响的下游文档。
8. 视频提示词请求遇到「输入参考图：无」或仍带「待补参考图」时，先路由分镜 owner 检查项目已有图片并刷新绑定。有匹配图就同请求续跑；有必要图缺失就列表停下。
...
9. 用户在会话里点名目标视频模型（“按 MiniMax H3 写”“用 Seedance 2.5”）而 `short-drama.json` 的
