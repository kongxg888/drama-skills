...
跨文档回流统一按 `$short-drama` 的“剧本到生产的联动回流”规则
分类。审查报告要指出问题属于提示词、分镜还是剧本层，以及会使哪些下游条目或生产 job 过期；不能只写“画面不对”或“声音不自然”。
...
- 人物、地点、道具状态和镜头起止是否连续；
- 新剧本是否在进入下游前完成可生产检查；存量剧本是否保留原文并只做有证据的定点修订；
- 提示词是否从准确起点到准确终点；
...
若审查对象包含已有图片、视频或 H3 结果，再检查结果观察是否正确回流：提示词问题留在提示词层，
镜头负载/时长/状态问题回到分镜层，台词/标点/人物意图/因果问题回到剧本层。已提交任务与新修订不可混为同一版本。
