...
- `overall_soundscape` 汇总环境声、物理音效和非语言人声，不重复对白；对白事件留在上面的主时间线。
  H3 支持在同一次生成中联合生成对白、环境声、动作音效和非画内配乐；因此默认把需要的四层都写清楚。
  只有剧本、分镜或创作者明确要求无配乐时，才写 `non_diegetic_music: N/A`。
...
H3 默认应明确写出需要的非画内配乐；只有剧本、分镜或创作者明确要求无配乐时，才保留
`non_diegetic_music: N/A`。提示词仍是生成指导，生成后要核听对白、环境声、动作音效和配乐是否实际出现、
层次是否互相遮挡。
...
**`craft_default`**：把对白的控制拆成三层：剧本原标点、`<d>` 外的自然语言发声描述、以及有依据时的时间段。
三层分别负责文字语气、声音表现和时序落点，不互相替代。
...
| `？` | 疑问或反问、句末上扬 |
| `！` | 强烈情绪、命令或惊讶；不要滥用 |
| `……` | 犹豫、吞吐、话未说完或断续；只在剧本原文如此时使用 |
| `——` | 打断、转折、强调或声音延长；只在剧本原文如此时使用 |
| `？！` | 疑问与强烈情绪并存；少量使用 |

逐字对白内必须保留剧本原标点，不为了“更有情绪”擅自改成多个感叹号、问号或省略号。需要控制人物
情绪时，在 `<d>` 外写可执行的发声方式，例如 `speaks in a low, clipped and dismissive voice`、
`answers after a short breath, calmly but firmly`、`briefly pauses after the rhetorical question`、
`raises her volume slightly on the final warning`。不要只写抽象的 `angry`、`sad` 或 `tense`。

H3 的 `<d>` 只容纳语言标签和剧本逐字台词；不要把 `[低声]`、`[愤怒]`、`<break>`、`<prosody>`、
`(叹气)` 或 `*动作*` 等其他语音系统的控制标记放进 `<d>`。这些标记可能被当作台词文字，或与 H3 的
结构发生冲突。语气词、叹词和口头填充词只有在剧本原文已有时才保留，不由提示词作者补写。

精确时间只有在分镜声音估时、已接受的音频或明确的剪辑依据支持时才写；否则用 `first`、`then`、
`after a short pause` 等自然语言顺序，避免伪造毫秒级精度。无论采用哪种写法，都要检查对白完整容纳在
镜头时长内，并在生成后核对逐字内容、停顿、重音、尾音和口型。
