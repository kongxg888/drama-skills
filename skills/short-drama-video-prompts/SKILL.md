...
进入本阶段时只读取当前《剧本.md》和《分镜.md》；聊天里的旧台词、旧提示词和旧参考图不作为来源。
若剧本或分镜在本轮发生改动，先按 `$short-drama` 的“剧本到生产的联动回流”规则
列出受影响的 `MOTION-*`，再写提示词。
...
猜，也不自行扩充新的禁用类别。时间线总结只列本镜实际有的声音层。
   ...
   声音同轨的目标模型若支持一次生成对白、环境声、动作音效和非画内配乐，默认把这四层写进同一条可复制提示词；
   非画内配乐只有在剧本、分镜或创作者明确要求无配乐时才写空。
   对白的原标点保留在逐字台词内，用标点提供基础停顿和句末语气；具体音色、音量、语速、重音、呼吸和停顿位置写在 `<d>` 外。
...
- 对白控制分三层：剧本原标点保留在 `<d>` 内；`<d>` 外用自然语言写发声方式；有可靠依据时再用时间段写开口、停顿和落点。不要把 `[低声]`、`[愤怒]`、`<break>`、`(叹气)` 或 `*动作*` 等其他系统的标记直接塞进 H3 的 `<d>`。
- 无对白镜同样说明底声或静默；时间线总结与实际声音层一致。对支持同轨音画的模型，默认明确写环境声、动作音效、对白和非画内配乐；
...
- 生成结果只暴露了语气、音量、节奏、构图、参考图用途或负面限制问题：在本技能修订当前 `MOTION-*`，建立新 job 预览。
- 生成结果暴露了动作容量、镜头时长、起终状态、空间或连续性问题：回到 `$short-drama-storyboard` 修订；分镜变化后，本镜和受影响相邻镜的 `MOTION-*` 视为 `stale/已过期`，重新读取分镜再写。
- 生成结果暴露了台词、标点、人物意图、因果或剧本事实问题：回到 `$short-drama-write` 修订；相关分镜、图片/视频提示词和已准备/已确认 job 全部视为 `stale/已过期`。
- 不在视频提示词里偷偷新增剧本没有的台词、语气词、对白解释或剧情事实；H3 的 `<d>` 必须来自当前剧本/分镜已确认的逐字文本。
- 已提交并计费的任务只收回结果，不撤销、不原样重投；修订后的提示词必须建立新 job 并重新预览、确认。
...
没有未解释的上游变更或 `stale/已过期` 条目后，实际生成才转 `$short-drama-produce`，先展示精确 job 并取得显式确认；本技能不调用外部服务。
