...
保留未受影响的镜头 ID。拆镜、并镜或改变镜头职责、时长、起点、终点或声音承载时，明确列出受影响的
图片关键帧和视频提示词，并将它们视为 `stale/已过期`，不能继续使用旧 job；需要刷新时只改受影响的
镜头，不自动重写用户没点名的其他文件。

生成图片或视频后发现问题时，先按 `$short-drama` 的“剧本到生产的联动回流”规则
判断：动作负载、时长、空间、起终状态和参考图绑定回到本技能；单纯的措辞或发声方式问题留给提示词 owner；
台词、人物意图或因果问题回到剧本 owner。分镜修订完成后，必须同步刷新受影响的视频提示词。
...
若本轮修改了分镜的动作、时长、起点、终点或声音承载，交付报告必须同时列出需刷新的下游条目，
不得把旧提示词继续标成可投产。
